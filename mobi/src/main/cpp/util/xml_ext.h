...
//media_type
    ...
    /***
     * 查找子元素， 根据提供的子元素名(或者第二个名字)，属性名/属性值
     * @param elem
     * @param child_name
     * @param second_child_name
     * @param attr_name
     * @param attr_value
     * @return
     */
    static const tinyxml2::XMLElement *getChildByTwoNameAndAttr(const tinyxml2::XMLElement *elem,
                                                                  const std::string &child_name,
    ...
    /****
     * 深度优先搜索，找打elem下面的的第一个tagName
     * @param elem
     * @param tagName
     * @return
     */
    static tinyxml2::XMLElement *findEleByTag(tinyxml2::XMLElement *elem, const char *tagName);


    /****
     * 获得某个元素的第一个指定名字（或者第二个名字） 的元素并返回
     * @param elem
     * @param tagName
     * @param secondName
     * @return
     */
    static tinyxml2::XMLElement *firstChildEleWithTwoName(tinyxml2::XMLElement *elem, const char *tagName, const char* secondName);

    /****
     * 获得某个元素的下一个指定名字（或者第二个名字）的兄弟节点
     * @param elem
     * @param tagName
     * @param secondName
     * @return
     */
    static tinyxml2::XMLElement *nextSiblingEleWithTwoName(tinyxml2::XMLElement *elem, const char *tagName, const char* secondName);

    /****
     * 深度优先搜索，找到第一个满足 callback 条件的元素
     * @param elem     起始元素（自身也会被检查）
     * @param callback 判断函数，返回 true 表示找到目标
     * @return 第一个满足条件的元素，未找到返回 nullptr
     */
    template<typename Callback>
    static tinyxml2::XMLElement *findEleByCallback(tinyxml2::XMLElement *elem, Callback &&callback) {
        tinyxml2::XMLElement *item = elem;
