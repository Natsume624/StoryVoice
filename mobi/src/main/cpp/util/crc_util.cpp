...
void CrcResults::process_files_crc(const std::vector<fs::path> &files, size_t start, size_t end) {
    ...
    for (size_t i = start; i < end && i < files.size(); ++i) {
        ...
        try {
            ...
            // ★ 同一次 IO 同时算 CRC32 + SHA-256
            Sha256Context shaCtx;
            Sha256Initialise(&shaCtx);
            ...
            while (file.read(buffer.data(), buffer.size())) {
                ...
                Sha256Update(&shaCtx, buffer.data(), static_cast<uint32_t>(gcount));
            }

            //处理剩余字节(如果有),同时喂 CRC + SHA
            auto remainder = file.gcount();
            for (size_t j = 0; j < remainder; ++j) {
                crc = (crc >> 8) ^ crc_table[(crc & 0xFF) ^ static_cast<uint8_t>(buffer[j])];
            }
            if (remainder > 0) {
                Sha256Update(&shaCtx, buffer.data(), static_cast<uint32_t>(remainder));
            }

            // SHA-256 完成 → hex 字符串
            SHA256_HASH shaDigest;
            Sha256Finalise(&shaCtx, &shaDigest);
            char hex[2 * SHA256_HASH_SIZE + 1];
            for (int k = 0; k < SHA256_HASH_SIZE; ++k) {
                snprintf(hex + k * 2, 3, "%02x", shaDigest.bytes[k]);
            }
            hex[2 * SHA256_HASH_SIZE] = '\0';

            add({filepath, crc ^ 0xFFFFFFFFu, std::string(hex)});

        } catch (const std::exception &e) {
            add({filepath, static_cast<uint32_t>(-1), ""}); //标记错误状态(-1),hash 留空
            continue;
        }
    }
}
