...
std::vector<std::string> zip_ext::inner_zip_files(unzFile uf) {
    ...
    while(index < gi.number_entry) {
        ...
        unz_file_info info;
        char file_name[UNZ_MAXFILENAMEINZIP + 1];
        memset(file_name, 0, sizeof(file_name));

        err = unzGetCurrentFileInfo(uf, &info, file_name, UNZ_MAXFILENAMEINZIP, nullptr, 0, nullptr, 0);
        if (err != UNZ_OK) {
            LOGE("%s get current file failed", __func__);
            unzCloseCurrentFile(uf);
            return ret;
        }
        if (info.size_filename >= UNZ_MAXFILENAMEINZIP) {
            LOGW("%s filename truncated (%u >= %d)", __func__, (unsigned)info.size_filename, UNZ_MAXFILENAMEINZIP);
        }
        file_name[UNZ_MAXFILENAMEINZIP] = '\0';
        if (file_name[0] != '\0') {
            ret.emplace_back(file_name);
        }
        ...
        if (err != UNZ_OK) {
            LOGE("%s goto next file failed (err=%d)", __func__, err);
            break;
        }
