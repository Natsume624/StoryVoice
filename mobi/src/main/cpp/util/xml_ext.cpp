 ...
 * @return
 */
const tinyxml2::XMLElement *xml_ext::getChildByTwoNameAndAttr(const tinyxml2::XMLElement *elem,
                                                           const std::string &child_name,
                                                           const std::string &second_child_name,
                                                           const std::string &attr_name,
                                                           const std::string &attr_value) {
    const tinyxml2::XMLElement *target = nullptr;
    if (elem != nullptr) {
        const tinyxml2::XMLElement *child = xml_ext::firstChildEleWithTwoName(
                const_cast<tinyxml2::XMLElement *>(elem), child_name.c_str(), second_child_name.c_str());
        ...
        while (child != nullptr) {
            ...
            child = xml_ext::nextSiblingEleWithTwoName(const_cast<tinyxml2::XMLElement *>(child), child_name.c_str(), second_child_name.c_str());
        }
    }
    return target;
}
...
int xml_ext::parseEpub3NcxData(std::string &nav_data, std::vector<NavPoint> &points, std::string &nav_path) {
    ...
    tinyxml2::XMLElement * navEle = findEleByCallback(bodyEle, [](tinyxml2::XMLElement *ele){
        const char *eleName = ele->Name();
        if (eleName == nullptr || strcasecmp(eleName, "nav") != 0) return false;
        std::string epubType = getEleAttr(ele, "epub:type");
        if (epubType == "toc") return true;
        std::string role = getEleAttr(ele, "role");
        return role == "doc-toc";
    });
    if (!navEle) {
        LOGE("%s failed parse epub3 nav, no nav[toc] element", __func__);
        return 0;
    }

    tinyxml2::XMLElement *olEle = navEle->FirstChildElement("ol");
    tinyxml2::XMLElement *liELe = navEle->FirstChildElement("li");
...
std::string xml_ext::ele_params(const tinyxml2::XMLElement *elem, std::string &spineSrcName) {
    ...
    if (elem != nullptr) {
        for (auto attri = elem->FirstAttribute(); attri != nullptr; attri = attri->Next()) {
            ...
            if (name == "href") {
                if (!value.empty()) {
                    if (!string_ext::startWith(value, "http")) { //非http的锚定链接，则将其转换成标准的锚点链接
                        value = string_ext::base_url_decode(value);
                        ...
                        if (string_ext::startWith(value, "#")) {
                            ...
                            href.append(value);
                        } else if (string_ext::startWith(string_ext::to_lower(value), "filepos:")) {
                            std::string num = value.substr(8);
                            if (!num.empty() && string_ext::is_number(num)) {
                                href.append(spineSrcName);
                                href.append("#");
                                href.append(num);
                            } else {
                        ...
                        } else if (string_ext::is_number(value)) {
                            href.append(spineSrcName);
                            href.append("#");
                            href.append(value);
                        } else {
...
int xml_ext::parse(
    ...
    while (flag && item != nullptr) {
        ...
        //优先遍历子节点
        auto child = item->FirstChild();
        if (child != nullptr && domElem != nullptr && !tags.empty()) { //当前节点是Element，并且孩子节点不为空，则深入下一层
        ...
        if (flag) {
            if (bro != nullptr) {
                item = bro;
            } else { //栈已空且当前层无兄弟节点，向上逐层查找祖先的兄弟节点
                auto current = item;
                while (current != nullptr) {
                    tinyxml2::XMLNode *parent = current->Parent();
                    if (parent == nullptr) {
                        flag = false;
                        break;
                    }
                    ...
                    if (ele_name == topParentTag) {
                        flag = false;
                        break;
                    }
                    auto parentSibling = ele_parent->NextSibling();
...
/***
 * 根据tagName 深度优先搜索第一个找到的 tagName 的节点
 * @param elem
 * @param id
 * @return
 */
tinyxml2::XMLElement *xml_ext::findEleByTag(tinyxml2::XMLElement *elem, const char *tagName) {
    return findEleByCallback(elem, [tagName](tinyxml2::XMLElement *ele) {
        const char *eleName = ele->Name();
        return eleName != nullptr && strcasecmp(eleName, tagName) == 0;
    });
}


tinyxml2::XMLElement *xml_ext::nextSiblingEleWithTwoName(tinyxml2::XMLElement *elem, const char *tagName, const char* secondName) {
    if (elem == nullptr) {
        return nullptr;
    }
    ...
    auto ele = elem->NextSiblingElement(tagName);
    if (ele == nullptr && (secondName != nullptr && strlen(secondName) > 0)) {
        ele = elem->NextSiblingElement(secondName);
    }
    return ele;
}

tinyxml2::XMLElement *xml_ext::firstChildEleWithTwoName(tinyxml2::XMLElement *elem, const char *tagName, const char* secondName) {
    if (elem == nullptr) {
        return nullptr;
    }
    if (tagName == nullptr || strlen(tagName) == 0) {
    ...
    auto ele = elem->FirstChildElement(tagName);
    if (ele == nullptr && (secondName != nullptr && strlen(secondName) > 0)) {
        ele = elem->FirstChildElement(secondName);
    }
    return ele;
}
