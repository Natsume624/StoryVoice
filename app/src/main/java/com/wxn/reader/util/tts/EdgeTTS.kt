...
import com.wxn.reader.util.tts.data.AudioFrame
...
class EdgeTTS {
    ...
    /**
     * At first, I used Frame to save metadata and data,
     * then I discovered that input must wait previous output done.
     */
    ...
    suspend fun input(text: String, metadata: AudioMetaData) {
        Logger.d("EdgeTTS:input:text[$text],metadata[$metadata]")
        if (metadata.invalid()) {
