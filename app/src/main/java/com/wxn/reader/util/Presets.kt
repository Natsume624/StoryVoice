    ...
    companion object {
        ...
        fun material(): List<List<Color>> {
            return listOf(
                ...
                readingWarmDark.map { getColor(it) },
                blueGray.map { getColor(it) },

                gray1.map { getColor(it) },
                readingLightWarm.map { getColor(it) },
                gray2.map { getColor(it) },

                other.map{ getColor(it) },
            )
        }
        ...
        val readingWarmDark = listOf(
            "#4A3C31",
            "#3A2C2C",
            "#2B2B2B",
        )

        val readingLightWarm = listOf(
            "#FDF5E6",
            "#FFF8E7",
            "#FAEBD7",
            "#F0F7E8"
