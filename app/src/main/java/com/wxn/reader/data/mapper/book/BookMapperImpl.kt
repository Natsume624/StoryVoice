...
class BookMapperImpl @Inject constructor(
    private val chineseConverter: BookChineseConverter
) : BookMapper {
...
//            fileType = stringToFileType(book.fileType),
    ...
    override suspend fun toBook(bookEntity: BookEntity): Book {
        return Book(
            id = bookEntity.id,
            title = chineseConverter.convert(bookEntity.title),
            author = chineseConverter.convert(bookEntity.authors.orEmpty()),
            ...
            favoriteDate = bookEntity.favoriteDate,
