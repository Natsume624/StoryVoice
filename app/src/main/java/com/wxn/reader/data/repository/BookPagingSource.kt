...
class BookPagingSource(val bookDao: BookDao,
    ...
    private val source = run {
        val statusList = readingStatuses.toList().takeIf { it.isNotEmpty() }
        val storageList = fileTypes.flatMap { it.storageValues() }
        bookDao.getAllBooksSorted(
            sortOption.name.lowercase(),
            isAscending,
            readingStatuses = statusList,
            allStatus = if (statusList.isNullOrEmpty()) 1 else 0,
