...
import com.wxn.bookparser.parser.txt.JuniversalCharsetDetector
import com.wxn.bookparser.parser.txt.TxtCharsetDetector
import com.wxn.bookparser.parser.txt.ChapterScanner
...
object ParserModule {
    ...
    @Provides
    @Singleton
    fun provideTxtCharsetDetector(): TxtCharsetDetector =
        JuniversalCharsetDetector()

    @Provides
    @Singleton
    fun provideTxtTextParser(
        markdownParser: MarkdownParser,
        chapterScanner: ChapterScanner,
        context: Context,
        // ★ v12 TXT 统一字节偏移方案：TxtTextParser 改造后依赖 TxtBookMetaStore（DB 侧抽象，
        // 由 app 模块提供 TxtBookMetaStoreImpl 实现）+ TxtCharsetDetector（已有 provideTxtCharsetDetector）。
        txtBookMetaStore: com.wxn.bookparser.parser.txt.TxtBookMetaStore,
        txtCharsetDetector: TxtCharsetDetector
    ): TxtTextParser = TxtTextParser(markdownParser, chapterScanner, context, txtBookMetaStore, txtCharsetDetector)
